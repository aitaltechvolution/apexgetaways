      ...
      <section className="relative pt-36 pb-24 overflow-hidden parallax-section" style={{background:'#F8F6F2'}}>
        ...
        <div className="relative container-pad text-center">
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}}>
            <span className="text-sm font-bold uppercase tracking-widest text-gold block mb-4">Where Will You Go?</span>
            <h1 className="font-display font-bold text-primary mb-5" style={{fontSize:'clamp(2.5rem,6vw,4rem)'}}>
              Explore Destinations
            </h1>
            <p className="text-lg max-w-xl mx-auto mb-8" style={{color:'#374151'}}>
            ...
            <div className="max-w-md mx-auto relative">
              ...
              <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search destinations…"
                className="w-full pl-11 pr-4 py-4 rounded-2xl text-base text-primary placeholder-gray-400 focus:outline-none"
        ...
        <div className="container-pad flex flex-wrap gap-2 justify-center">
          {REGIONS.map(r=>(
            <button key={r} onClick={()=>setRegion(r)}
              ...
              style={{
                background:region===r?'linear-gradient(135deg,#C9A84C,#F5C842)':'#F3F4F6',
                color:region===r?'#0A1628':'#374151',
                border:region===r?'none':'1px solid #E5E7EB',
              }}>{r}</button>
          ))}
        </div>
      </div>
        ...
        <div className="container-pad">
          {filtered.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-primary font-bold mb-1">No destinations found</p>
              <p className="text-base" style={{color:'#4B5563'}}>Try a different search or region</p>
            </div>
          ) : (
                  ...
                  <Link to={`/destinations/${dest.id}`}
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-5">
                      <div className="flex items-center gap-1.5 mb-1">
                        ...
                        <p className="text-sm" style={{color:'#374151'}}>{dest.country}</p>
                      </div>
                      <h3 className="font-display font-bold text-primary text-xl leading-tight">{dest.name}</h3>
                      <div className="flex items-center justify-between mt-2">
                        <p className="text-sm font-semibold" style={{color:'#C9A84C'}}>{dest.from}</p>
                      ...
                      <div className="mt-2 overflow-hidden max-h-0 group-hover:max-h-16 transition-all duration-300">
                        <div className="flex flex-wrap gap-1 pt-2">
                          {dest.attractions.slice(0,3).map(a=>(
                            <span key={a} className="text-[12px] px-2 py-0.5 rounded-full"
                              style={{background:'#E5E7EB',color:'#111827'}}>{a}</span>
                          ))}
                        </div>
                      </div>
                    </div>
          ...
          <div className="mt-16 p-8 rounded-2xl text-center reveal"
            ...
            <h3 className="font-display font-bold text-primary text-xl mb-3">Don't See Your Destination?</h3>
