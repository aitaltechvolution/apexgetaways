            ...
            <div className="flex items-end justify-between flex-wrap gap-4">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="px-3 py-1 rounded-full text-sm font-bold" style={{background:'rgba(201,168,76,0.9)',color:'#0A1628'}}>{dest.tag}</span>
                </div>
                <h1 className="font-display font-bold text-primary mb-1" style={{fontSize:'clamp(2.2rem,5vw,3.5rem)'}}>{dest.name}</h1>
                <div className="flex items-center gap-3 text-base" style={{color:'#1F2937'}}>
              ...
              <div className="text-right">
                <p className="text-sm" style={{color:'#4B5563'}}>Packages from</p>
        ...
        <div className="container-pad grid lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="reveal p-6 rounded-2xl" style={{background:'#FFFFFF',border:'1px solid #E5E7EB',boxShadow:'0 1px 3px rgba(10,22,40,0.05)'}}>
              <h2 className="font-display font-bold text-primary text-xl mb-4">Top Attractions</h2>
              <div className="grid grid-cols-2 gap-3">
                {dest.attractions.map(a=>(
                  <div key={a} className="flex items-center gap-2 p-3 rounded-xl" style={{background:'rgba(201,168,76,0.06)',border:'1px solid rgba(201,168,76,0.12)'}}>
                    ...
                    <span className="text-base font-medium text-primary">{a}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Travel requirements */}
            <div className="reveal p-6 rounded-2xl" style={{background:'#FFFFFF',border:'1px solid #E5E7EB',boxShadow:'0 1px 3px rgba(10,22,40,0.05)'}}>
              <h2 className="font-display font-bold text-primary text-xl mb-4">Travel Requirements</h2>
              <ul className="space-y-3">
                {reqs.map(r=>(
                  <li key={r} className="flex items-start gap-3">
                    <CheckCircle size={15} className="shrink-0 mt-0.5" style={{color:'#C9A84C'}}/>
                    <span className="text-base" style={{color:'#1F2937'}}>{r}</span>
                  </li>
                ))}
              </ul>
              <div className="mt-4 p-4 rounded-xl" style={{background:'rgba(201,168,76,0.06)',border:'1px solid rgba(201,168,76,0.15)'}}>
                <p className="text-sm font-bold text-primary mb-1"> Need Visa Assistance?</p>
                <p className="text-sm mb-2" style={{color:'#4B5563'}}>Apex Getaways provides full visa documentation support and guidance for all destinations.</p>
                <Link to="/services/visa" className="text-sm font-bold" style={{color:'#C9A84C'}}>Learn about our Visa Service →</Link>
              </div>
            </div>
            ...
            {related.length > 0 && (
              <div className="reveal">
                <h2 className="font-display font-bold text-primary text-xl mb-4">Packages to {dest.name}</h2>
                  ...
                  {related.map(pkg=>(
                    <Link key={pkg.id} to={`/packages/${pkg.id}`}
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-base text-primary truncate group-hover:text-gold transition-colors">{pkg.title}</p>
                        <p className="text-sm mt-0.5" style={{color:'#4B5563'}}>{pkg.nights} nights</p>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="font-display font-bold text-base" style={{color:'#C9A84C'}}>{formatNGN(pkg.price)}</p>
          ...
          <div className="reveal">
            <div className="sticky space-y-4" style={{top:'88px'}}>
              <div className="p-6 rounded-2xl" style={{background:'#F3F4F6',border:'1px solid rgba(201,168,76,0.2)'}}>
                <p className="font-display font-bold text-primary text-xl mb-1">Plan Your Trip</p>
                <p className="text-sm mb-5" style={{color:'#4B5563'}}>Get a custom quote for {dest.name}</p>
                <Link to={`/contact?dest=${encodeURIComponent(dest.name)}`} className="block w-full text-center btn-gold mb-3">
                  Get Free Quote
                </Link>
                <Link to="/packages" className="block w-full text-center py-3 rounded-xl text-base font-bold border-2 transition-all"
              ...
              <a href={`https://wa.me/${BRAND.whatsapp.replace('+','')}`} target="_blank" rel="noreferrer"
                className="flex items-center gap-3 p-4 rounded-2xl font-bold text-primary text-base transition-all hover:opacity-90"
                style={{background:'#25D366'}}>
                <Phone size={16}/> Chat on WhatsApp — 24/7
              </a>
              <div className="p-4 rounded-xl text-sm" style={{background:'rgba(201,168,76,0.06)',border:'1px solid rgba(201,168,76,0.15)',color:'#4B5563'}}>
                <p className="font-bold text-primary mb-1"> All Inclusive</p>
