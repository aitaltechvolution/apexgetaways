...
const iStyle = (focused) => ({
  ...
  border: focused ? '1px solid #C9A84C' : '1px solid #E5E7EB',
...
function ApexInput({ label, ...props }) {
  ...
  return (
    <div>
      <label style={{ display:'block', fontSize:'11px', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', color:'#4B5563', marginBottom:'8px' }}>{label}</label>
      ...
      <section className="relative pt-36 pb-20 overflow-hidden" style={{background:'#F8F6F2'}}>
        ...
        <div className="relative container-pad text-center">
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}}>
            <span className="text-sm font-bold uppercase tracking-widest text-gold block mb-4">Get In Touch</span>
            <h1 className="font-display font-bold text-primary leading-tight mb-4" style={{fontSize:'clamp(2.5rem,6vw,4rem)'}}>
              Contact Our Team
            </h1>
      ...
      <section className="section-pad" style={{background:'#F8F6F2'}}>
        <div className="container-pad grid lg:grid-cols-5 gap-14">
          ...
          <div className="lg:col-span-3 reveal">
            <h2 className="font-display font-bold text-primary text-2xl mb-2">Send a Message</h2>
            ...
            {sent ? (
              <motion.div initial={{scale:0.9,opacity:0}} animate={{scale:1,opacity:1}}
                ...
                <p className="text-base max-w-xs" style={{color:'#4B5563'}}>
              ...
              <form onSubmit={submit} className="space-y-5">
                ...
                <ApexSelect label="Service Needed" name="service" value={form.service} onChange={h}>
                  ...
                  {SERVICES.map(s => <option key={s.id} value={s.title} style={{background:'#FFFFFF'}}>{s.icon} {s.title}</option>)}
                </ApexSelect>

                <div>
                  <label style={{display:'block',fontSize:'11px',fontWeight:700,textTransform:'uppercase',letterSpacing:'0.1em',color:'#4B5563',marginBottom:'8px'}}>
                  ...
                  <select name="interest" value={form.interest} onChange={h}
                    ...
                    <option value="" style={{background:'#FFFFFF'}}>— Optional: select a package or destination —</option>
                    <optgroup label="Tour Packages" style={{background:'#FFFFFF'}}>
                      {PACKAGES.map(p=><option key={p.id} value={p.title} style={{background:'#FFFFFF'}}>{p.flag} {p.title}</option>)}
                    </optgroup>
                    <optgroup label="Destinations" style={{background:'#FFFFFF'}}>
                      {DESTINATIONS.map(d=><option key={d.id} value={`Trip to ${d.name}`} style={{background:'#FFFFFF'}}>{d.flag} Trip to {d.name}</option>)}
                    </optgroup>
                    <option value="Custom Package" style={{background:'#FFFFFF'}}> Custom itinerary</option>
                    <option value="Visa Enquiry" style={{background:'#FFFFFF'}}> Visa Enquiry</option>
                    <option value="Study Abroad" style={{background:'#FFFFFF'}}> Study Abroad</option>
                    <option value="Immigration" style={{background:'#FFFFFF'}}> Immigration Consultation</option>
                  </select>
                </div>
          ...
          <div className="lg:col-span-2 reveal right space-y-5">
            <h2 className="font-display font-bold text-primary text-2xl mb-6">Contact Details</h2>
            ...
            {[
              ...
              <div key={label} className="flex items-start gap-4 p-5 rounded-2xl"
                ...
                <div>
                  <p className="font-bold text-sm text-primary mb-0.5">{label}</p>
                  {href ? (
                    <a href={href} className="text-base transition-colors" style={{color:'#374151'}}
                      onMouseEnter={e=>e.currentTarget.style.color='#C9A84C'}
                      onMouseLeave={e=>e.currentTarget.style.color='#374151'}>{val}</a>
                  ) : (
                    <p className="text-base" style={{color:'#374151'}}>{val}</p>
                  )}
                </div>
              </div>
            ))}
            ...
            <a href={`https://wa.me/${BRAND.whatsapp.replace('+','')}`} target="_blank" rel="noreferrer"
              ...
              <div>
                <p className="font-bold text-primary text-base">Chat on WhatsApp</p>
                <p className="text-sm text-primary/70">Instant response · Available 24/7</p>
              </div>
            </a>

            {/* Hours */}
            <div className="p-5 rounded-2xl" style={{background:'rgba(201,168,76,0.06)',border:'1px solid rgba(201,168,76,0.2)'}}>
              <p className="font-bold text-primary mb-2 text-base">Business Hours</p>
              <p className="text-base font-bold" style={{color:'#C9A84C'}}>{BRAND.hours}</p>
              <p className="text-sm mt-0.5" style={{color:'#4B5563'}}>We're always available for you</p>
            </div>

            {/* Social */}
            <div>
              <p className="font-bold text-primary text-base mb-3">Follow Us</p>
              <div className="flex flex-col gap-2">
                <a href={BRAND.instagram} target="_blank" rel="noreferrer"
                  ...
                  <div>
                    ...
                    <p className="text-[12px]" style={{color:'#6B7280'}}>@apex_getaways_travel_ltd</p>
                  </div>
                </a>
                <a href={BRAND.tiktok} target="_blank" rel="noreferrer"
                  ...
                  <div>
                    <p className="text-sm font-bold text-primary">TikTok</p>
                    <p className="text-[12px]" style={{color:'#6B7280'}}>@visa_travel_support</p>
                  </div>
                </a>
              </div>
            </div>
