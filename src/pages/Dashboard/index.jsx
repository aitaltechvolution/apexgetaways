...
import { subscribeUserBookings, updateUserDoc, uploadPassport } from '../../lib/firebase'
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0"
              style={{ background: isActive ? 'rgba(201,168,76,0.12)' : 'rgba(255,255,255,0.05)' }}>
              <TypeIcon size={18} style={{ color: isActive ? '#C9A84C' : 'rgba(255,255,255,0.3)' }} />
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap mb-1">
                <span className="font-bold text-white text-sm capitalize">{booking.bookingType} Booking</span>
          ...
          <div className="flex items-center gap-3 shrink-0">
            {booking.total > 0 && <p className="font-bold text-sm" style={{ color: '#C9A84C' }}>{formatNGN(booking.total)}</p>}
        ...
        {expanded && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
            className="mt-4 pt-4 space-y-1.5 text-xs overflow-hidden" style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            {booking.selectedSeats?.length > 0 && <p style={{ color: 'rgba(255,255,255,0.55)' }}>Seats: {booking.selectedSeats.join(', ')}</p>}
            {booking.baggage?.outbound && <p style={{ color: 'rgba(255,255,255,0.55)' }}>Baggage: {booking.baggage.outbound.label}</p>}
            {booking.addons?.insurance && <p style={{ color: 'rgba(255,255,255,0.55)' }}>Travel insurance included</p>}
            {booking.paymentRef && <p style={{ color: 'rgba(255,255,255,0.4)' }}>Payment: {booking.paymentRef}</p>}
            ...
            {(booking.status === 'confirmed' || booking.status === 'tickets_issued') && (
              <a href={booking.ticketUrl || '#'} className="inline-flex items-center gap-1.5 mt-2 px-3 py-2 rounded-lg text-xs font-bold"
...
function ProfileTab({ userDoc, user }) {
  ...
  const blr = e => { e.target.style.borderColor = 'rgba(255,255,255,0.1)' }
  ...
  const F = (label, field, props = {}) => (
    <div>
      <label className="block text-[10px] font-bold uppercase tracking-wider mb-1.5" style={{ color: 'rgba(255,255,255,0.35)' }}>{label}</label>
  ...
  return (
    <div className="p-6 rounded-2xl" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
      ...
      <div className="mt-4 p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px dashed rgba(255,255,255,0.1)' }}>
        <p className="text-xs font-bold text-white mb-2">Passport Scan</p>
        <input ref={fileRef} type="file" accept="image/*,.pdf" className="hidden" onChange={handlePassport}/>
        {form.passportUrl ? (
          <div className="flex items-center gap-2">
            <CheckCircle size={14} style={{ color: '#22c55e' }}/>
            <span className="text-xs text-green-400">Passport on file</span>
            <a href={form.passportUrl} target="_blank" rel="noreferrer" className="text-xs underline ml-1" style={{ color: '#C9A84C' }}>View</a>
            <button onClick={() => fileRef.current?.click()} className="text-xs underline ml-auto" style={{ color: 'rgba(255,255,255,0.4)' }}>Replace</button>
          </div>
        ) : (
      ...
      <button onClick={save} disabled={saving}
        className="w-full btn-gold py-3 font-bold text-sm mt-4 flex items-center justify-center gap-2">
...
export default function DashboardPage() {
  ...
  return (
    <>
      ...
      <div className="min-h-screen pt-16" style={{ background: '#070D1A' }}>

        {/* Header */}
        <div className="py-8" style={{ background: '#0A1628', borderBottom: '1px solid rgba(201,168,76,0.12)' }}>
          <div className="container-pad">
            <div className="flex items-center justify-between flex-wrap gap-4 mb-5">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="font-display font-bold text-white text-xl">{userDoc?.displayName || 'Traveller'}</h1>
                  <p className="text-sm" style={{ color: 'rgba(255,255,255,0.4)' }}>{user.email}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                {isAdmin && (
                  <Link to="/admin" className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold"
                    style={{ background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.25)', color: '#f87171' }}>
                    <Shield size={12}/> Admin Panel
                  </Link>
                )}
                <button onClick={logout} className="flex items-center gap-1.5 text-xs font-bold px-3 py-2 rounded-xl"
            ...
            <div className="flex gap-3">
              {[['Total', bookings.length], ['Active', active.length], ['Completed', completed.length]].map(([l, v]) => (
                <div key={l} className="text-center px-4 py-2.5 rounded-xl" style={{ background: 'rgba(255,255,255,0.05)' }}>
        ...
        {/* Tab bar */}
        <div className="sticky z-20 py-2" style={{ top: '64px', background: 'rgba(7,13,26,0.97)', backdropFilter: 'blur(24px)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            {[['bookings', 'My Bookings'], ['profile', 'My Profile']].map(([val, label]) => (
              <button key={val} onClick={() => setTab(val)}
                ...
                style={{
                  ...
                  color: tab === val ? '#0A1628' : 'rgba(255,255,255,0.5)',
                }}>{label}</button>
            ))}
            <button onClick={() => navigate('/booking')}
              className="ml-auto flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-xs font-bold"
          ...
          {tab === 'bookings' && (
            bookings.length === 0 ? (
              <div className="text-center py-20">
                <Plane size={48} className="mx-auto mb-4 opacity-20 text-white"/>
                <h3 className="font-display font-bold text-white text-xl mb-2">No bookings yet</h3>
                <p className="text-sm mb-6" style={{ color: 'rgba(255,255,255,0.4)' }}>Start planning your next trip.</p>
