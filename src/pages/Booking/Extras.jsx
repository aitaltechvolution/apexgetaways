  ...
  return (
    <button type="button" onClick={() => onSelect(option)}
      ...
      style={{
        ...
        border: `2px solid ${isSelected ? '#C9A84C' : 'rgba(255,255,255,0.08)'}`,
        transform: isSelected ? 'scale(1.02)' : 'scale(1)',
      }}>
      <div className="flex items-center justify-between w-full mb-2">
        <Luggage size={18} style={{ color: isSelected ? '#C9A84C' : 'rgba(255,255,255,0.4)' }} />
        {isSelected && <Check size={15} style={{ color: '#C9A84C' }} />}
      </div>
      <p className="font-bold text-sm text-white">{option.label}</p>
      <p className="text-xs mt-0.5" style={{ color: 'rgba(255,255,255,0.4)' }}>{option.sub}</p>
      <p className="font-bold text-sm mt-2" style={{ color: option.price === 0 ? '#22c55e' : '#C9A84C' }}>
    ...
    <>
      ...
      <section className="py-10" style={{ background: '#070D1A', minHeight: '80vh' }}>
        <div className="container-pad max-w-4xl mx-auto">
          <h1 className="font-display font-bold text-white text-2xl mb-2">Add Extras</h1>
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-7 gap-2">
                {MEAL_PREFS.map(m => (
                  <button key={m.id} type="button" onClick={() => setMealPref(m.id)}
                    ...
                    style={{
                      background: mealPref === m.id ? 'rgba(201,168,76,0.12)' : 'rgba(255,255,255,0.04)',
                      border: `2px solid ${mealPref === m.id ? '#C9A84C' : 'rgba(255,255,255,0.08)'}`,
                    }}>
                    <Utensils size={14} style={{ color: mealPref === m.id ? '#C9A84C' : 'rgba(255,255,255,0.35)' }}/>
                    <span className="text-[10px] font-semibold text-center" style={{ color: mealPref === m.id ? '#C9A84C' : 'rgba(255,255,255,0.55)' }}>{m.label}</span>
                  </button>
                ))}
              </div>
            </Section>
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {[
                  ...
                  <button key={String(opt.val)} type="button" onClick={() => setInsurance(opt.val)}
                    ...
                    style={{
                      background: insurance === opt.val ? 'rgba(201,168,76,0.08)' : 'rgba(255,255,255,0.04)',
                      border: `2px solid ${insurance === opt.val ? '#C9A84C' : 'rgba(255,255,255,0.08)'}`,
                    }}>
                    ...
                    <div>
                      ...
                      <p className="font-bold text-sm mt-1" style={{ color: opt.price > 0 ? '#C9A84C' : 'rgba(255,255,255,0.4)' }}>
            ...
            <Section title=" Special Assistance" sub="Wheelchair, visual/hearing impairment, unaccompanied minor">
              <label className="flex items-center gap-3 cursor-pointer">
                <div className="relative">
                  ...
                  <div className="w-12 h-6 rounded-full transition-colors" style={{ background: assistance ? '#C9A84C' : 'rgba(255,255,255,0.15)' }} />
                ...
                <span className="text-sm font-medium text-white">I require special assistance</span>
              </label>
              {assistance && (
                <p className="mt-3 text-xs p-3 rounded-xl" style={{ background: 'rgba(201,168,76,0.08)', color: '#C9A84C', border: '1px solid rgba(201,168,76,0.2)' }}>
          ...
          <div className="mt-8 p-5 rounded-2xl" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(201,168,76,0.2)' }}>
            {fd && (
              <div className="space-y-2 mb-4 text-sm">
                ...
                <div className="flex justify-between font-bold pt-2" style={{ borderTop: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={proceed} className="flex-1 btn-gold py-3.5 flex items-center justify-center gap-2 font-bold text-sm">
...
function Section({ title, sub, children }) {
  return (
    <div className="p-6 rounded-2xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
      ...
      {sub && <p className="text-xs mb-4" style={{ color: 'rgba(255,255,255,0.4)' }}>{sub}</p>}
      {children}
    </div>
  )
}
...
export function StepBar({ step }) {
  ...
  return (
    <div className="sticky top-0 z-30 py-3" style={{ background: 'rgba(10,22,40,0.97)', borderBottom: '1px solid rgba(201,168,76,0.12)', backdropFilter: 'blur(24px)' }}>
          ...
          <div key={s} className="flex items-center shrink-0">
            <div className="flex flex-col items-center">
              <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all"
                style={{
                  background: i < step ? '#22c55e' : i === step ? 'linear-gradient(135deg,#C9A84C,#F5C842)' : 'rgba(255,255,255,0.08)',
                  color: i < step ? 'white' : i === step ? '#0A1628' : 'rgba(255,255,255,0.3)',
                }}>
                {i < step ? '' : i + 1}
              </div>
              <span className="text-[10px] mt-1 font-medium hidden sm:block"
                style={{ color: i === step ? '#C9A84C' : i < step ? '#22c55e' : 'rgba(255,255,255,0.3)' }}>{s}</span>
            </div>
            {i < steps.length - 1 && <div className="w-8 sm:w-16 h-px mx-1" style={{ background: i < step ? '#22c55e' : 'rgba(255,255,255,0.1)' }} />}
          </div>
        ))}
      </div>
    </div>
