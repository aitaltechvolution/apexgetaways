    ...
    <section className="section-pad bg-surface-light dark:bg-surface-dark" ref={ref}>
      <div className="container-pad">
        <div className="flex items-end justify-between mb-12">
          ...
          <Link to="/packages" className="hidden sm:flex items-center gap-1.5 text-sm font-bold text-primary hover:gap-2.5 transition-all">View All <ArrowRight size={14} /></Link>
        </div>
              ...
              <motion.div key={pkg.id} initial={{opacity:0,y:24}} animate={inView?{opacity:1,y:0}:{}} transition={{delay:i*0.1}}>
                <Link to={`/packages/${pkg.id}`} className="group block bg-white dark:bg-card-dark rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-800 shadow-card hover:shadow-card-hover hover:-translate-y-1 transition-all duration-300">
                  <div className="relative overflow-hidden h-48">
                    ...
                    <span className="absolute top-3 left-3 bg-red-500 text-white text-xs font-bold px-2.5 py-1 rounded-lg">Save {save}%</span>
                    <span className="absolute top-3 right-3 bg-accent text-navy text-xs font-bold px-2.5 py-1 rounded-lg">{pkg.tag}</span>
                  ...
                  <div className="p-5">
                    ...
                    <ul className="space-y-1 mb-4">
                      {pkg.includes.map(inc => (
                        <li key={inc} className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    <div className="flex items-end justify-between pt-3 border-t border-gray-100 dark:border-gray-800">
                      <div>
                        <p className="text-xs text-gray-400 line-through">{formatNGN(pkg.oldPrice)}</p>
                        <p className="font-bold text-primary text-lg">{formatNGN(pkg.price)}</p>
                      </div>
                      <span className="text-xs font-bold text-primary flex items-center gap-1">Book Now <ArrowRight size={12} /></span>
                    </div>
                  </div>
                </Link>
              </motion.div>
